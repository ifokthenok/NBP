...
extern void gralloc_lock_hook(int bufferIndex, int* bufferStates);
extern void gralloc_unlock_hook(int bufferIndex, int* bufferStates);
        ...
        void** vaddr)
{
    ...
    *vaddr = (void*)hnd->base;
	...
	// Hook gralloc_lock()
	if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
		gralloc_lock_hook(hnd->index, m->bufferStates);
	}
        ...
        buffer_handle_t handle)
{
    ...
    private_handle_t* hnd = (private_handle_t*)handle;
	if (hnd->flags & private_handle_t::PRIV_FLAGS_FRAMEBUFFER) {
		gralloc_unlock_hook(hnd->index, m->bufferStates);
	}
