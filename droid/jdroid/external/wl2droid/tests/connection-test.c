...
setup(int *s)
{
	...
	assert(os_socketpair_cloexec(AF_UNIX, SOCK_STREAM, 0, s) == 0);
...
{
	//assert(socketpair(AF_UNIX, SOCK_STREAM | SOCK_CLOEXEC, 0, data->s) == 0);
	assert(os_socketpair_cloexec(AF_UNIX, SOCK_STREAM, 0, data->s) == 0);
...
TEST(connection_marshal_demarshal)
{
	...
	char f[] = "/cache/1000/wayland-tests-XXXXXX";
...
TEST(connection_marshal_alot)
{
	...
	for (i = 0; i < 2000; i++) {
		strcpy(f, "/cache/1000/wayland-tests-XXXXXX");
